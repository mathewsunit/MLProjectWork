...
for(i in 1:5){
  ...
  trainCtl <- train(Class ~ ., data = trainData,method = "rpart",tuneLength=20,trControl = trainControl(method = "repeatedcv", repeats = 7))
  rpartPred <- predict(trainCtl,testData)
  #print(confusionMatrix(rpartPred,testData$V37))
  print(sqrt(mean(rpartPred-testData$Class)^2))
  vectord <- c(vectord,sqrt(mean(rpartPred-testData$Class)^2))
  fancyRpartPlot(trainCtl$finalModel)
}